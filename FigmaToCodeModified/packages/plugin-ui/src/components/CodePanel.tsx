  ...
  useEffect(() => {
    if (!isEmpty) {
      const sendCodeToAPI = async () => {
        ...
        try {
          const response = await fetch("https://epic-name.onrender.com/store_data", {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
            },
            body: JSON.stringify({"name": "1", "payload": code }),
          });

          if (!response.ok) {
            console.error("Failed to send code to the API");
          }
        } catch (error) {
          console.error("Error sending code to the API:", error);
        }
      };
